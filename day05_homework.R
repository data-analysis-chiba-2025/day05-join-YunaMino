...
answer_1 <- flights |> left_join(planes, join_by(tailnum))
...
missing_data = flights |> anti_join(planes, by = "tailnum")
answer_3 <- missing_data |> count() |> pull(n)==0
...
answer_5 <- answer_4 |> group_by(name) |> summarise(ave_delay = mean(arr_delay, na.rm = TRUE))
